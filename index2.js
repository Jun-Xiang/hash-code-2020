...
libraryDetails.sort((a, b) => b.totalBooks / b.ship - a.totalBooks / a.ship);
...
fs.appendFileSync('./res2.txt', `${res.totalLib}\n`);
res.libOrder.forEach((lib, i) => {
    fs.appendFileSync('./res2.txt', `${lib} ${res.booksToSend[i].length} \n${res.booksToSend[i].toString().replace(/,/g, ' ')} \n`);
})